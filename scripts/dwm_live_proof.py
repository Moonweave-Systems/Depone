...
from compile_workflow import canonical_hash, compile_plan, read_json, write_json_atomic, write_text_atomic  # noqa: E402
...
def resolve_repo_input(value: str | Path, *, code: str) -> Path:
    ...
    try:
        ...
        raise LiveProofError(code, "input must resolve inside this repository", path=value) from exc
...
def resolve_manifest_out(value: str | Path) -> Path:
    ...
    try:
        ...
        raise LiveProofError("ERR_LIVE_PROOF_OUT_UNSAFE", f"manifest output must resolve under {root}", path=value) from exc
    if resolved == root:
        raise LiveProofError("ERR_LIVE_PROOF_OUT_UNSAFE", "manifest output must name a directory", path=value)
    check_components_not_symlink(candidate, code="ERR_LIVE_PROOF_PATH_SYMLINK")
    return resolved
...
def resolve_live_out(value: str | Path) -> Path:
    ...
    try:
        ...
        raise LiveProofError("ERR_LIVE_PROOF_OUT_UNSAFE", f"live output must resolve under {root}", path=value) from exc
    if resolved == root:
        raise LiveProofError("ERR_LIVE_PROOF_OUT_UNSAFE", "live output must name a directory", path=value)
    check_components_not_symlink(candidate, code="ERR_LIVE_PROOF_PATH_SYMLINK")
    return resolved
...
def prepare_owned_dir(path: Path, proof_id: str, *, source: Path | str, root: Path) -> None:
    if path.exists():
        if path.is_symlink():
            raise LiveProofError("ERR_LIVE_PROOF_PATH_SYMLINK", "output is a symlink", path=path)
        if not path.is_dir():
            raise LiveProofError("ERR_LIVE_PROOF_OUT_UNSAFE", "output exists and is not a directory", path=path)
        sentinel = read_sentinel(path)
        if sentinel is None or sentinel.get("proof_id") != proof_id:
            raise LiveProofError("ERR_LIVE_PROOF_OUT_UNSAFE", "existing output is not V102-owned", path=path)
        shutil.rmtree(path)
    root.mkdir(parents=True, exist_ok=True)
    path.mkdir(parents=True)
    write_json_atomic(
        path / SENTINEL,
        {"tool": TOOL, "schema_version": SCHEMA_VERSION, "proof_id": proof_id, "source_path": str(source), "created_at": now_utc()},
        root=path,
    )
...
def safe_copy_seed(seed: Path, workspace: Path) -> None:
    for path in [seed, *seed.rglob("*")]:
        if path.is_symlink():
            raise LiveProofError("ERR_LIVE_PROOF_PATH_SYMLINK", "seed must not contain symlinks", path=path)
    shutil.copytree(seed, workspace)


def run_process(argv: list[str], cwd: Path, *, input_text: str | None = None, timeout_seconds: int = 120) -> subprocess.CompletedProcess[str]:
    env = dict(os.environ)
...
def git_text(args: list[str], cwd: Path) -> str:
    ...
    if result.returncode != 0:
        raise LiveProofError("ERR_LIVE_PROOF_GIT_FAILED", result.stderr.strip() or result.stdout.strip(), path=cwd)
...
def init_seed_repo(workspace: Path) -> dict[str, str]:
    ...
    run_process(["git", "config", "user.email", "live-proof@example.invalid"], workspace, timeout_seconds=30)
    run_process(["git", "config", "user.name", "DWM Live Proof"], workspace, timeout_seconds=30)
    run_process(["git", "add", "."], workspace, timeout_seconds=30)
    commit = run_process(["git", "commit", "-m", "seed"], workspace, timeout_seconds=30)
    if commit.returncode != 0:
        raise LiveProofError("ERR_LIVE_PROOF_GIT_FAILED", commit.stderr.strip() or commit.stdout.strip(), path=workspace)
    return {"head": git_text(["rev-parse", "HEAD"], workspace), "status": git_text(["status", "--short"], workspace)}
...
def command_hash(command: list[str]) -> str:
    return canonical_hash({"argv": command, "backend": "codex-cli", "mode": "installed-codex"})


def tree_hash(root: Path) -> str:
    files = []
    for path in sorted(item for item in root.rglob("*") if item.is_file()):
        files.append({"path": path.relative_to(root).as_posix(), "sha256": canonical_hash({"content": path.read_text()})})
    return canonical_hash({"files": files})


def base_bundle(proof_id: str, *, seed_path: str = "fixtures/live-proof/seed", objective: str = "Make the seeded pytest check pass.") -> dict[str, Any]:
    return {
        "proof_id": proof_id,
        ...
        "worktree": {"path": f"out/live-proofs/{proof_id}/workspace", "isolated": True, "pre_state": {}, "post_state": {}},
        "prompt_hash": "0" * 64,
        "packet_hash": "1" * 64,
        "adapter_hash": "2" * 64,
        "commands": [],
        ...
        "verification": {"command": "python -m pytest -q", "returncode": 0, "passed": True, "before": "red", "after": "green", "output_path": "verification-after.txt"},
        "review": {"decision": "approved", "evidence_path": "review.json"},
        "elapsed_seconds": 1.0,
        "interruptions": 0,
        ...
        "dogfood_comparison": {
            "mode": "dwm-controlled",
            "status": "run",
            "metrics": {"elapsed_seconds": 1.0, "interruptions": 0, "verification_passed": True},
        },
        "decision": "live-proof-pass",
        "blocked_by": [],
        "claim_policy": CLAIM_POLICY,
        "source_hashes": {"seed": "3" * 64, "plan": "4" * 64, "packet": "1" * 64, "prompt": "0" * 64},
    }
...
def validate_bundle(bundle: dict[str, Any]) -> list[str]:
    ...
    for key in ["task", "worktree", "verification", "review", "dogfood_comparison", "source_hashes"]:
    ...
    dogfood = bundle.get("dogfood_comparison") if isinstance(bundle.get("dogfood_comparison"), dict) else {}
    ...
    if bundle["decision"] == "live-proof-pass":
        if verification.get("passed") is not True or verification.get("before") != "red" or verification.get("after") != "green":
            errors.append("verification.red_green")
        if dogfood.get("status") != "run" or metrics.get("verification_passed") is not True:
...
def render_markdown(bundle: dict[str, Any], errors: list[str] | None = None) -> str:
    ...
    if bundle.get("blocked_by"):
        lines.append(f"- Blocked by: `{', '.join(str(item) for item in bundle['blocked_by'])}`")
    if errors:
        lines.extend(["", "## Schema Errors", *[f"- `{error}`" for error in errors]])
    lines.extend(["", "This bundle proves only the recorded V102 state. It makes no direct-agent superiority claim.", ""])
    return "\n".join(lines)
...
def write_bundle(out_dir: Path, bundle: dict[str, Any]) -> list[str]:
    ...
    write_text_atomic(out_dir / "live-proof.md", render_markdown(bundle, errors), root=out_dir)
    write_text_atomic(out_dir / "live-proof.sha256", canonical_hash(bundle) + "\n", root=out_dir)
    write_json_atomic(out_dir / "status.json", {"decision": bundle.get("decision"), "valid": not errors, "errors": errors}, root=out_dir)
    return errors


def run_manifest(manifest_path: Path, out_dir: Path) -> dict[str, Any]:
    manifest_path = resolve_repo_input(manifest_path, code="ERR_LIVE_PROOF_MANIFEST_INVALID")
    if manifest_path != PUBLIC_MANIFEST.resolve(strict=False):
        raise LiveProofError("ERR_LIVE_PROOF_MANIFEST_INVALID", "public manifest execution is limited to fixtures/v102/manifest.json", path=manifest_path)
    manifest = read_json(manifest_path)
    fixtures = manifest.get("fixtures")
    if not isinstance(fixtures, list):
        raise LiveProofError("ERR_LIVE_PROOF_MANIFEST_INVALID", "manifest fixtures must be a list", path=manifest_path)
    ...
    for fixture in fixtures:
        ...
        bundle = fixture.get("bundle") if isinstance(fixture.get("bundle"), dict) else {}
        ...
        if expected_decision is not None and bundle.get("decision") != expected_decision:
            fixture_errors.append(f"expected decision {expected_decision}, got {bundle.get('decision')}")
        records.append(
            {
                "id": fixture_id,
    ...
    if failed_required:
        raise LiveProofError("ERR_LIVE_PROOF_FIXTURE_FAILED", "required live-proof fixture failed", path=manifest_path)
    return summary
...
def self_test() -> None:
    ...
    blocked.update(
        {
            "decision": "blocked",
            "blocked_by": [{"code": "ERR_EXEC_BACKEND_AUTH", "message": "Codex CLI authentication failed"}],
            "verification": {"command": "python -m pytest -q", "returncode": None, "passed": False, "before": "red", "after": "not-run", "output_path": "verification-before.txt"},
            "dogfood_comparison": {"mode": "dwm-controlled", "status": "blocked", "metrics": {"elapsed_seconds": 0.0, "interruptions": 0, "verification_passed": False}},
        }
    )
    ...
    failed.update(
        {
            "decision": "failed",
            "verification": {"command": "python -m pytest -q", "returncode": 1, "passed": False, "before": "red", "after": "red", "output_path": "verification-after.txt"},
            "dogfood_comparison": {"mode": "dwm-controlled", "status": "run", "metrics": {"elapsed_seconds": 1.0, "interruptions": 0, "verification_passed": False}},
        }
    )
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    if not args.i_approve_live_codex:
        raise LiveProofError("ERR_LIVE_PROOF_APPROVAL_REQUIRED", "live codex execution requires --i-approve-live-codex")
    codex = shutil.which("codex")
    if codex is None:
        raise LiveProofError("ERR_EXEC_BACKEND_UNAVAILABLE", "codex binary is not on PATH")
    timeout_seconds = int(args.timeout_seconds)
    if timeout_seconds < 1 or timeout_seconds > 3600:
        raise LiveProofError("ERR_LIVE_PROOF_TIMEOUT_INVALID", "timeout_seconds must be between 1 and 3600")
    ...
    write_text_atomic(out_dir / "verification-before.txt", red.stdout + red.stderr, root=out_dir)
    ...
    if dirty_before_exec.strip():
        ...
        bundle.update(
            {
                "decision": "blocked",
                "blocked_by": [{"code": "ERR_EXEC_DIRTY_WORKTREE", "message": "seed workspace became dirty before Codex execution"}],
                "worktree": {"path": rel(workspace), "isolated": True, "pre_state": pre_state, "post_state": {"status": dirty_before_exec}},
                "verification": {"command": "python -m pytest -q", "returncode": red.returncode, "passed": False, "before": "red", "after": "not-run", "output_path": "verification-before.txt"},
                "dogfood_comparison": {"mode": "dwm-controlled", "status": "blocked", "metrics": {"elapsed_seconds": 0.0, "interruptions": 0, "verification_passed": False}},
            }
        )
    ...
    if red.returncode == 0:
        ...
        bundle.update(
            {
                "decision": "blocked",
                "blocked_by": [{"code": "ERR_LIVE_PROOF_RED_CHECK_NOT_RED", "message": "seed verification passed before Codex ran"}],
                "worktree": {"path": rel(workspace), "isolated": True, "pre_state": pre_state, "post_state": {"status": git_text(["status", "--short"], workspace)}},
                "verification": {"command": "python -m pytest -q", "returncode": red.returncode, "passed": False, "before": "green", "after": "not-run", "output_path": "verification-before.txt"},
                "dogfood_comparison": {"mode": "dwm-controlled", "status": "blocked", "metrics": {"elapsed_seconds": 0.0, "interruptions": 0, "verification_passed": False}},
            }
        )
        write_bundle(out_dir, bundle)
    ...
    if packet_status != "ready":
        raise LiveProofError("ERR_LIVE_PROOF_PACKET_NOT_READY", f"compiled packet status is {packet_status}", path=run_dir)
    ...
    write_text_atomic(out_dir / "verification-after.txt", green.stdout + green.stderr, root=out_dir)
    post_state = {"head": git_text(["rev-parse", "HEAD"], workspace), "status": git_text(["status", "--short"], workspace), "diff_stat": git_text(["diff", "--stat"], workspace)}
    ...
    if codex_auth_failed(result):
        ...
        blocked_by.append({"code": "ERR_EXEC_VERIFY_FAILED", "message": "verification failed after Codex execution"})
    ...
    bundle = {
        ...
        "schema_version": SCHEMA_VERSION,
        "tool": TOOL,
        "task": {"id": "live-proof-1", "seed_path": rel(seed), "objective": compiled["packet"]["objective"], "verification_command": "python -m pytest -q"},
        "backend": "codex-cli",
        "mode": "installed-codex",
        "model_provider": "codex-cli-default",
        ...
        "commands": [{"argv": command, "cwd": rel(workspace), "timeout_seconds": timeout_seconds}],
        "files_touched": touched_files(workspace),
        "transcript_path": "transcript.md",
        "verification": {"command": "python -m pytest -q", "returncode": green.returncode, "passed": green.returncode == 0, "before": "red", "after": "green" if green.returncode == 0 else "red", "output_path": "verification-after.txt"},
        "review": {"decision": "approved" if verification_passed else "request-changes", "evidence_path": "verification-after.txt"},
        ...
        "dogfood_comparison": {
            ...
            "metrics": {"elapsed_seconds": elapsed, "interruptions": 0, "verification_passed": verification_passed},
        },
        "decision": decision,
...
def inspect_proof(path: Path) -> dict[str, Any]:
    ...
    if not bundle_path.is_file() or bundle_path.is_symlink():
        raise LiveProofError("ERR_LIVE_PROOF_INPUT_MISSING", "live-proof.json is missing", path=path)
    ...
    summary = {
        ...
        "valid": not errors,
        "errors": errors,
        "verification_passed": bundle.get("verification", {}).get("passed") if isinstance(bundle.get("verification"), dict) else None,
        "dogfood_status": bundle.get("dogfood_comparison", {}).get("status") if isinstance(bundle.get("dogfood_comparison"), dict) else None,
    }
    return summary
...
def main() -> None:
    ...
    try:
        ...
        if args.manifest:
            if args.out is None:
                raise LiveProofError("ERR_LIVE_PROOF_OUT_REQUIRED", "--out is required with --manifest")
            print(json.dumps(run_manifest(args.manifest, args.out), sort_keys=True))
            return
        if args.command == "run":
            bundle = run_live(args)
            print(json.dumps({"proof_id": bundle["proof_id"], "decision": bundle["decision"]}, sort_keys=True))
            return
        if args.command == "inspect":
            print(json.dumps(inspect_proof(args.proof), sort_keys=True))
            return
        raise LiveProofError("ERR_LIVE_PROOF_COMMAND_REQUIRED", "use --self-test, --manifest, run, or inspect")
